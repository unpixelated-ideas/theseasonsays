...
assert.equal(await page.locator('.footer-links [aria-disabled="true"]').count(),5);assert.equal(await page.locator('.footer-links a[href]').count(),0);await page.goto('http://127.0.0.1:5173/#complete');assert.ok(await page.locator('.annual-list article').count()>17);await page.locator('#year').selectOption('2024');assert.match(await page.locator('.reference .eyebrow').innerText(),/2024/);
for(const r of ['about','privacy','terms','feedback']){await page.goto(`http://127.0.0.1:5173/#${r}`);assert.ok((await page.locator('.information').innerText()).length>30);}
